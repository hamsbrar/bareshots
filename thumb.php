...
final class timthumb
{
    private $cacheDirectory = '';

    private $cachefile = '';

    private $cropTop = false;

    private static $curlDataWritten = 0;

    private static $curlFH = false;

    private $docRoot = '';

    private $errors = array();

    private $fileCacheVersion = 1; //Generally if timthumb.php is modifed (upgraded) then the salt changes and all cache files are recreated. This is a backup mechanism to force regen.

    private $filePrependSecurityBlock = '<?php die(\'Execution denied!\'); //'; //Designed to have three letter mime type, space, question mark and greater than symbol appended. 6 bytes total.
    ...
    public function __construct()
    {
        ...
        if (FILE_CACHE_DIRECTORY)
        {
            ...
            if ( ! \touch($this->cacheDirectory . '/index.html'))
            {
        ...
        if (BLOCK_EXTERNAL_LEECHERS && \array_key_exists('HTTP_REFERER', $_SERVER) && ( ! \preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', $_SERVER['HTTP_REFERER'])))
        {
            ...
            \header('Expires: ' . \gmdate('D, d M Y H:i:s', \time()));
            echo $imgData;
    ...
    private function cleanCache()
    {
        ...
        if (@\filemtime($lastCleanFile) < (\time() - FILE_CACHE_TIME_BETWEEN_CLEANS))
        { //Cache was last cleaned more than 1 day ago
            ...
            if ($files)
            {
                $timeAgo = \time() - FILE_CACHE_MAX_FILE_AGE;
    ...
    private function getLocalImagePath($src)
    {
    ...
    private function getURL($url, $tempfile)
    {
    ...
    {
        return (bool) (\sizeof($this->errors) > 0);
    }
    ...
    private function param($property, $default = '')
    {
    ...
    private function processImageAndWriteToCache($localImage)
    {
        ...
        $imgType = '';
        $tempfile = \tempnam($this->cacheDirectory, 'timthumb_tmpimg_');
        ...
        {
            $exec = PNGCRUSH_PATH;
            $tempfile2 = \tempnam($this->cacheDirectory, 'timthumb_tmpimg_');
        ...
        $tempfile4 = \tempnam($this->cacheDirectory, 'timthumb_tmpimg_');
    ...
    private function sanityFail($msg)
    {
    ...
    private function sendImageHeaders($mimeType, $dataSize)
    {
    ...
    private function serveCacheFile()
    {
    ...
    private function serveExternalImage()
    {
        ...
        $tempfile = \tempnam($this->cacheDirectory, 'timthumb');
    ...
    private function serveImg($file)
    {
        $s = \getimagesize($file);
        if ( ! ($s && $s['mime']))
        {
    ...
    private function serveInternalImage()
    {
    ...
    private function serveWebshot()
    {
        ...
        $tempfile = \tempnam($this->cacheDirectory, 'timthumb_webshot');
        $url = $this->src;
    ...
    private function toDelete($name)
    {
        $this->debug(3, "Scheduling file {$name} to delete on destruct.");
        $this->toDeletes[] = $name;
    }

    private function tryBrowserCache()
    {
        if (BROWSER_CACHE_DISABLE)
        {
    ...
    private function tryServerCache()
    {
        ...
        if (\file_exists($this->cachefile))
        {
            ...
            if ($this->isURL)
            {
                ...
                if (\filesize($this->cachefile) < 1)
                {
                    ...
                    if (\time() - @\filemtime($this->cachefile) > WAIT_BETWEEN_FETCH_ERRORS)
                    {
