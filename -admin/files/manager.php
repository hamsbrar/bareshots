...
\define('APP_TITLE', 'Admin Panel - ' . $_SESSION['editing_profile']);

$use_auth = false;

$auth_users = $readonly_users = array();
...
$root_path = $_SESSION['editing_path'];

$root_url = $_SESSION['root_path'];

$http_host = $_SERVER['HTTP_HOST'];

$directories_users = array('root' => $_SESSION['editing_path']);

$iconv_input_encoding = 'UTF-8';

$datetime_format = 'd.m.y H:i';

$allowed_file_extensions = KNOWN_EXTENSIONS;

$allowed_upload_extensions = KNOWN_EXTENSIONS;

$favicon_path = '';

$exclude_items = array('*.json');
...
// Default language
$lang = $cfg->data['lang'] ?? 'en';

// Show or hide files and folders that starts with a dot
$show_hidden_files = $cfg->data['show_hidden'] ?? true;

// PHP error reporting - false = Turns off Errors, true = Turns on Errors
$report_errors = $cfg->data['error_reporting'] ?? true;

// Hide Permissions and Owner cols in file-listing
$hide_Cols = $cfg->data['hide_Cols'] ?? true;

// Show directory size: true or speedup output: false
$calc_folder = $cfg->data['calc_folder'] ?? true;
...
if ($ip_ruleset != 'OFF')
{
    ...
    if ($proceed == false)
    {
        \trigger_error('User connection denied from: ' . $clientIp, E_USER_WARNING);
...
if ($use_auth)
{
    ...
    elseif (isset($_POST['fm_usr'], $_POST['fm_pwd']))
    {
        ...
        {
            if (isset($auth_users[$_POST['fm_usr']], $_POST['fm_pwd']) && \password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']]))
            {
                ...
                <div class="row justify-content-md-center h-100">
                    <div class="card-wrapper">
                        ...
                        <div class="footer text-center">
                            &mdash;&mdash; &copy;
                            <a href="https://tinyfilemanager.github.io/" target="_blank" class="text-muted" data-version="<?php echo _VERSION; ?>">CCP Programmers</a> &mdash;&mdash;
                        </div>
                    </div>
                </div>
            </div>
...
{
    $root_path = $directories_users[$_SESSION[FM_SESSION_ID]['logged']] ?? $root_path;
}
...
if (isset($_POST['ajax']) && ! FM_READONLY)
{
    ...
    if (isset($_POST['type']) && $_POST['type'] == 'upload' && ! empty($_REQUEST['uploadurl']))
    {
        ...
        $use_curl = false;
        $temp_file = \tempnam(\sys_get_temp_dir(), 'upload-');
        $fileinfo = new stdClass();
        $fileinfo->name = \trim(\basename($url), ".\x00..\x20");
...
// Create folder
if (isset($_GET['new'], $_GET['type']) && ! FM_READONLY)
{
    $type = $_GET['type'];
    ...
    if (\fm_isvalid_filename($new) && $new != '' && $new != '..' && $new != '.')
    {
        ...
        if ($_GET['type'] == 'file')
        {
            if ( ! \file_exists($path . '/' . $new))
            {
                ...
                {
                    @\fopen($path . '/' . $new, 'w') || exit('Cannot open file:  ' . $new);
...
if (isset($_GET['copy'], $_GET['finish']) && ! FM_READONLY)
{
    ...
    if ($from != $dest)
    {
        $msg_from = \trim(FM_PATH . '/' . \basename($from), '/');
        if ($move)
...
if (isset($_GET['copy']) && ! isset($_GET['finish']) && ! FM_READONLY)
{
    ...
    foreach ($folders as $f)
    {
        ?>
                <li>
                    <a href="?p=<?php echo \urlencode(\trim(FM_PATH . '/' . $f, '/')); ?>&amp;copy=<?php echo \urlencode($copy); ?>"><i class="fa fa-folder-o"></i> <?php echo \fm_convert_win($f); ?></a></li>
                <?php
    } ?>
        </ul>
    </div>
            ...
            <?php if ( ! $quickView) { ?>
                ...
                <p class="break-word">
                    ...
                    if ($is_image)
                    {
                        ...
                        echo 'Image sizes: ' . ($image_size[0] ?? '0') . ' x ' . ($image_size[1] ?? '0') . '<br>';
                    }
                    // Text info
                    if ($is_text)
                    {
                ...
                <p>
                    ...
                    {
                        ?>
                        <b><a href="?p=<?php echo \urlencode(\trim(FM_PATH)); ?>&amp;edit=<?php echo \urlencode($file); ?>" class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo \lng('Edit'); ?>
                            </a></b> &nbsp;
                        <b><a href="?p=<?php echo \urlencode(\trim(FM_PATH)); ?>&amp;edit=<?php echo \urlencode($file); ?>&env=ace"
    ...
    <div class="path">
        <div class="row">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-right pt-1">
                <a title="Back" class="btn btn-sm btn-outline-primary" href="?p=<?php echo \urlencode(\trim(FM_PATH)); ?>&amp;view=<?php echo \urlencode($file); ?>"><i class="fa fa-reply-all"></i> <?php echo \lng('Back'); ?></a>
                <a title="Backup" class="btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="backup('<?php echo \urlencode(\trim(FM_PATH)); ?>','<?php echo \urlencode($file); ?>')"><i class="fa fa-database"></i> <?php echo \lng('BackUp'); ?></a>
                <?php if ($is_text) { ?>
                    <?php if ($isNormalEditor) { ?>
                        <a title="Advanced" class="btn btn-sm btn-outline-primary" href="?p=<?php echo \urlencode(\trim(FM_PATH)); ?>&amp;edit=<?php echo \urlencode($file); ?>&amp;env=ace"><i class="fa fa-pencil-square-o"></i> <?php echo \lng('AdvancedEditor'); ?></a>
                    ...
                    <?php }
                        ...
                        <a title="Plain Editor" class="btn btn-sm btn-outline-primary" href="?p=<?php echo \urlencode(\trim(FM_PATH)); ?>&amp;edit=<?php echo \urlencode($file); ?>"><i class="fa fa-text-height"></i> <?php echo \lng('NormalEditor'); ?></a>
...
<form action="" method="post" class="pt-3">
        ...
        <table class="table table-bordered table-hover table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            foreach ($folders as $f)
            {
                ...
                <tr>
                    ...
                    <td>
                        <div class="filename"><a href="?p=<?php echo \urlencode(\trim(FM_PATH . '/' . $f, '/')); ?>"><i class="<?php echo $img; ?>"></i> <?php echo \fm_convert_win(\fm_enc($f)); ?>
                    ...
                    <?php if ( ! FM_IS_WIN && ! $hide_Cols) { ?>
                        <td><?php if ( ! FM_READONLY) { ?><a title="Change Permissions" href="?p=<?php echo \urlencode(FM_PATH); ?>&amp;chmod=<?php echo \urlencode($f); ?>"><?php echo $perms; ?></a><?php }
                else { ?><?php echo $perms; ?><?php } ?>
                        </td>
                    ...
                    <td class="inline-actions"><?php if ( ! FM_READONLY) { ?>
                            ...
                            <a title="<?php echo \lng('CopyTo'); ?>..." href="?p=&amp;copy=<?php echo \urlencode(\trim(FM_PATH . '/' . $f, '/')); ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
                        <?php } ?>
            ...
            foreach ($files as $f)
            {
                ...
                <tr>
                    ...
                    <?php if ( ! FM_IS_WIN && ! $hide_Cols) { ?>
                        <td><?php if ( ! FM_READONLY) { ?><a title="<?php echo 'Change Permissions'; ?>" href="?p=<?php echo \urlencode(FM_PATH); ?>&amp;chmod=<?php echo \urlencode($f); ?>"><?php echo $perms; ?></a><?php }
                else { ?><?php echo $perms; ?><?php } ?>
                        </td>
                    ...
                    <td class="inline-actions">
                        ...
                        <?php if ( ! FM_READONLY) { ?>
                            ...
                            <a title="<?php echo \lng('CopyTo'); ?>..."
                               href="?p=<?php echo \urlencode(FM_PATH); ?>&amp;copy=<?php echo \urlencode(\trim(FM_PATH . '/' . $f, '/')); ?>"><i class="fa fa-files-o"></i></a>
                        <?php } ?>
    ...
    <div class="row">
        ...
        <div class="col-3 d-none d-sm-block"><a href="https://tinyfilemanager.github.io" target="_blank" class="float-right text-muted"><?php echo $_SESSION['path']; ?> </a></div>
        <?php }
            else { ?>
            <div class="col-12"><a href="https://tinyfilemanager.github.io" target="_blank" class="float-right text-muted"><?php echo $_SESSION['path']; ?></a></div>
        <?php } ?>
    </div>
...
function fm_get_mime_type($file_path)
{
    ...
    if ( ! \stristr(\ini_get('disable_functions'), 'shell_exec'))
    {
        ...
        return \shell_exec('file -bi ' . $file);
    }
...
function fm_is_exclude_items($file)
{
    ...
    return (bool) ( ! \in_array($file, FM_EXCLUDE_ITEMS) && ! \in_array("*.{$ext}", FM_EXCLUDE_ITEMS));
}
...
function fm_get_size($file)
{
    ...
    if ($exec_works)
    {
        ...
        @\exec($cmd, $output);
        if (\is_array($output) && \ctype_digit($size = \trim(\implode("\n", $output))))
        {
            return $size;
        }
    }
...
function fm_get_filesize($size)
{
    ...
    return \sprintf('%s %s', \round($size / 1024 ** $power, 2), $units[$power]);
}
...
function fm_download_file($fileLocation, $fileName, $chunkSize = 1024)
{
    ...
    if ($size == 0)
    {
        exit('Zero byte file! Aborting download');
    }
...
final class FM_Zipper_Tar
{
    ...
    public function create($filename, $files)
    {
        ...
        return (bool) ($this->addFileOrDir($files));
    }
    ...
    public function unzip($filename, $path)
    {
        ...
        return (bool) ($this->tar->extractTo($path));
    }
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-expand-lg <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar; ?>">
        <a class="navbar-brand" href="<?php echo $_SESSION['panel_url'] . $_SESSION['back_path']; ?>"> <i class="fa fa-chevron-left"></i> <?php echo $_SESSION['back_title']; ?> </a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <?php
            $path = \fm_clean_path($path);
    $root_url = '<a href=\'?p=\'>' . $_SESSION['path'] . '</a>';
...
<?php if (isset($_GET['edit'], $_GET['env']) && FM_EDIT_FILE)
        {
            $ext = 'javascript';
...
function lng($txt)
{
    ...
    // English Language
    $tr['en']['AppName'] = $_SESSION['editing_profile'];
    $tr['en']['AppTitle'] = $_SESSION['editing_profile'] . '/' . $_SESSION['path'];
    $tr['en']['Login'] = 'Sign in';
